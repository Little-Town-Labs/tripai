...
function CheckoutEntry({ review }: { review: PlanReviewModel }) {
  if (review.trip.purchasedAt) {
    return (
      <section className="rounded-md border border-stone-300 bg-white p-4 shadow-sm">
        <h2 className="text-lg font-semibold">Trip unlocked</h2>
        <p className="mt-2 text-sm leading-6 text-stone-700">
          Your purchased trip is ready for the passenger-seat co-pilot view with stop handoffs and route context.
        </p>
        <Link
          href={`/app/trips/${review.trip.id}`}
